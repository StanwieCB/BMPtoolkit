...
void _BMP::Load(const char *fileName)
{
...
void _BMP::Save(const char *fileName) 
{
...
void _BMP::DownSample(int fac = 1)
{
	factor = fac;
...
//count the RMSE between this BMP and the target reference
double _BMP::RMSE(_BMP & ref)
{
	assert(width == ref.getWidth());
	assert(height == ref.getHeight());
	...
	for (int y = 0; y < height; ++y)
	{
		for (int x = 0; x < width; ++x)
		{
			unsigned char R1 = ImgValue_s[3 * (width*y + x) + 2];
			unsigned char G1 = ImgValue_s[3 * (width*y + x) + 1];
			unsigned char B1 = ImgValue_s[3 * (width*y + x) + 0];

			unsigned char R2 = ref.ImgValue_s[3 * (width*y + x) + 2];
			unsigned char G2 = ref.ImgValue_s[3 * (width*y + x) + 1];
			unsigned char B2 = ref.ImgValue_s[3 * (width*y + x) + 0];

			double diffR = pow(abs(R1 - R2), 2);
			double diffG = pow(abs(G1 - G2), 2);
			double diffB = pow(abs(B1 - B2), 2);
			MSE = MSE + diffR + diffG + diffB;
		}
	}
